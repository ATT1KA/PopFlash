...
import { portfolioAllocation, portfolioSummary, topAssets } from '@/data/mock';
        ...
        <Section title="Flagged Movers" subtitle="Outliers vs. historical 30-day drifts">
          <View style={{ gap: spacing.md }}>
            {topAssets.slice(0, 3).map((asset) => {
              const share = ((asset.value / portfolioSummary.totalValue) * 100).toFixed(1);

              return (
                <TouchableOpacity
                  key={asset.id}
                  activeOpacity={0.85}
                  onPress={() => router.push(`/portfolio/${asset.id}`)}
